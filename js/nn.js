...
const costs_bp = {
	...
	"mse": (AL,Y) => [[math.multiply(-2/AL[0].length, math.sum(math.subtract(Y,AL)))]]
}
	...
	predict(xs){
  	...
  	initializeParameters(nnShape){
  		...
  		for(var layer=1; layer < this.layers.length; layer++){
  			parameters['W' + layer] = Array.from(Array(this.layers[layer][0]), () => Array.from(Array(this.layers[layer-1][0])).map(x=>weightInitialize(this.layers[layer][0])))
